...
<head>
	...
	<script type="text/javascript" src="static/js/hx/UpdateHx.js"></script>
	<link rel="stylesheet" href="static/css/hx/UpdateHx.css">
	...
	<script type="text/javascript">
		...
		function showInfo(){
			$("#year").find("option[value = '${heyear}']").attr("selected", true);
			$("#month").find("option[value = '${hemonth}']").attr("selected", true);
			$("#date").find("option[value = '${heday}']").attr("selected", true);
			$("#years").find("option[value = '${hbyear}']").attr("selected", true);
			$("#months").find("option[value = '${hbmonth}']").attr("selected", true);
			$("#dates").find("option[value = '${hbday}']").attr("selected", true);
		}
	</script>
</head>
...
<div class="all">
	...
	<form id="searchForm" class="layui-form" action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="uid" value="${hx.uid}">
		<input type="hidden" name="hid" value="${hx.hid}">
		<input type="hidden" name="haddress" value="${hx.haddress}">
		<div class="layui-form-item">
			<label class="layui-form-label">合同名称：</label>
			<div class="layui-input-block">
				<input type="text" name="hname" required lay-verify="required" placeholder="请输入合同名称"
					   autocomplete="off" class="layui-input" value="${hx.hname}">
			</div>
		</div>
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" name="hobject" required lay-verify="required" placeholder="请输入合作对象"
					   autocomplete="off" class="layui-input" value="${hx.hobject}">
			</div>
		</div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" id="hmoney" name="hmoney" required lay-verify="required" placeholder="单位（元）"
					   autocomplete="off" class="layui-input" value="${hx.hmoney}">
			</div>
		</div>
		...
		<div class="xia">
			...
			<div class="layui-form-item1">
				<div class="layui-input-block">
					<button type="button" class="layui-btn" onclick="confirmAdd('UpdateHxInfo')" lay-submit
